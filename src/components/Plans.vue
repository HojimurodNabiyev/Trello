    ...
    Sortable.create(taskList.value, {
        ...
        onEnd(evt) {
            ...
            massiveeee.forEach(async (obj, index) => {
                await axios.patch(`http://localhost:10000/plan/${obj.id}`, planStore.planMassiv[index])
            })
        }
    })
})
    ...
    taskRefs.value.forEach(el => {
        ...
        Sortable.create(el, {
            ...
            onAdd(evt) {
                ...
                axios.put(`http://localhost:10000/plan/${planStore.planMassiv[fromIndex].id}`, planStore.planMassiv[fromIndex])
                axios.put(`http://localhost:10000/plan/${planStore.planMassiv[toIndex].id}`, planStore.planMassiv[toIndex])
            }
        })
    })
})
